...
def get_papi_auth_headers(api_key: str, api_key_id: str) -> dict:
    """
    Generate authentication headers for Palo Alto Networks API requests (Standard key).
    Uso estático: una sola vez. Preferir get_papi_auth_headers_factory para uso en MCP.
    """
    # Documentación y ejemplos Cortex usan "x-xdr-auth-id"; algunos backends son sensibles a mayúsculas.
    return {
...
def get_papi_auth_headers_advanced(api_key: str, api_key_id: str) -> dict:
    """
    Generate Advanced authentication headers for Cortex XDR API.
    Uses HMAC-SHA256: Authorization = sha256(api_key + nonce + timestamp).
    Nonce and timestamp change per request to prevent replay attacks.
    """
...
def bundle_openapi_from_folders():
    ...
    """
    # Solo incluir directorios que existan (remote_components puede no existir hasta que se ejecute update)
    openapi_dirs = [
        base_dir / "openapi"
        for base_dir in [BUILTINS_DIR, CUSTOM_DIR, REMOTE_DIR]
        if (base_dir / "openapi").exists()
    ]
