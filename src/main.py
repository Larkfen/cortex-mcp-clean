...
from usecase.fetcher import set_startup_credentials
...
async def async_main(transport: Transport):
    ...
    # Signal handlers are Unix-only; add_signal_handler raises NotImplementedError on Windows.
...
async def initialize_mcp_server(api_key: str, api_key_id: str, papi_url: str) -> FastMCP:
    set_startup_credentials(api_key, api_key_id)
    config = get_config()
    auth_type = getattr(config, "papi_auth_type", "standard") or "standard"
    base_url = get_papi_url(papi_url)
    if auth_type == "advanced":
        logger.info("Using Advanced API key (HMAC SHA256 per request)")
        shared_client = AdvancedPAPIClient(base_url, api_key, api_key_id)
    else:
        shared_client = PAPIClient(base_url, get_papi_auth_headers(api_key, api_key_id))
    mcp = create_mcp_server(api_key, api_key_id, shared_client)

    discover_and_register_modules(mcp)

    spec = bundle_openapi_from_folders()
    if not spec or not spec.get("paths"):
        logger.warning("No OpenAPI spec or paths loaded; custom/builtin tools may be missing. Check that builtin_components/openapi and custom_components/openapi exist.")
    else:
        path_count = len(spec.get("paths", {}))
        logger.info(f"OpenAPI spec loaded with {path_count} path(s) (builtin + custom_components + remote_components)")
    open_api_mcp = FastMCP.from_openapi(
        spec or {"paths": {}, "components": {"schemas": {}}},
        shared_client,
    )
